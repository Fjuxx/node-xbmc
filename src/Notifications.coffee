...
class Notifications
  ...
  @delegate: (data) =>
    ...
    @api.pubsub.emit "notification:#{type}", data
    if @[type]?
  ...
  @stop: (data = null) =>
    debug 'stop', data
    @api.pubsub.emit 'api:playerStopped', data
  ...
  @inputrequested: =>
    ...
    @api.pubsub.emit 'api:Input.OnInputRequested'

  @inputfinished: =>
    debug 'inputfinished'
    @api.pubsub.emit 'api:Input.OnInputFinished'
